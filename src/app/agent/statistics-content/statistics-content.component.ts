...
export class StatisticsContentComponent implements OnInit {
  ...
  @Input() set header(value: string) { this.headerChange(value); }
  @Input() set time(value: string) { this.timeChange(value); }
  @Input() set agent(value: string) { this.agentChange(value); }

  @Input() set statistics(value: any) {
    if(value !== undefined && value.data !== undefined 
      && value.head !== undefined && value.bars !== undefined) {
  ...
  agentChange(selected){
    switch (selected) {
      case "all":
        ...
        break; 
      default:
        this.agentService.getAgentInformation(localStorage.getItem('agent-help-chat-token'), selected)
        .subscribe(
          (data: any) => { this._agent = data.user.firstname + " " + data.user.lastname; },
