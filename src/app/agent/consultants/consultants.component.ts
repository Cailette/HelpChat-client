...
export class ConsultantsComponent implements OnInit {
  ...
  onFormSubmit(form: NgForm) {
    if (this.isAdding) this.add(form);
    if (this.isEditing) this.edit(form);
  }
  ...
  add(form: NgForm) {
    this.agentService.createAgent(
      localStorage.getItem('agent-help-chat-token'), form.value)
      .subscribe(
        (data: any) => {
          this.getAgents();
          this.isAdding = false;
          this.isAddedSuccess = true;
          setTimeout(() => { this.isAddedSuccess = false; }, 3000);
        },
  ...
  edit(form: NgForm) {
    this.agentService.updateAgent(
      localStorage.getItem('agent-help-chat-token'), this.user._id, form.value)
      .subscribe(
        (data: any) => {
          this.getAgents();
          this.isEditing = false;
          this.isEditedSuccess = true;
          setTimeout(() => { this.isEditedSuccess = false; }, 3000);
        },
        (err: HttpErrorResponse) => { this.isEmailError = true; }
      );
  }
  ...
  editSelectedAgent(agentId: string) {
    this.agentService.getAgentInformation(
      localStorage.getItem('agent-help-chat-token'), agentId)
      .subscribe(
        (data: any) => {
          this.user = data.user;
          this.isEditing = true;
  ...
  delete() {
    this.agentService.deleteAgent(
      localStorage.getItem('agent-help-chat-token'), this.deleteAgent)
      .subscribe(
        (data: any) => { this.getAgents(); },
        (err: HttpErrorResponse) => {  this.isDataError = true; }
      );
  }
}